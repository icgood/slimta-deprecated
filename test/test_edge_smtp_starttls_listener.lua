...
local function check_messages(messages)
    ...
    assert("ESMTPS" == message.client.protocol)
    assert("test" == message.client.ehlo)
    assert("127.0.0.1" == message.client.ip)
    assert("TLS" == message.client.security)
...
function send_smtp(host, port)
    ...
    local ehlo = client:ehlo("test_pretls")
    assert(ehlo.code == "250")

    local starttls = client:starttls()
    assert(starttls.code == "220")
    ...
    local got_cert, verified, host_matched = enc:verify_certificate(host)
